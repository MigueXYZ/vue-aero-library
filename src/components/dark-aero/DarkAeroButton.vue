<template>
    <button
      :style="{
        ...
        '--initial-glow': 'rgba(128, 128, 128, 0.5)',  /* cinza inicial */
        '--glow-color': resolvedGlowColor,             /* o valor real do glow */
  ...
  // se o usuário não passou glowColor, usamos props.color
  const resolvedGlowColor = computed(() => {
    return props.glowColor.trim() !== '' 
      ? props.glowColor 
      : props.color
  })
  ...
  button {
    ...
    /* transição lenta e suave do glow */
    transition:
  ...
  button:hover:not(:disabled) {
    /* agora usamos a cor real do glow */
    box-shadow:
